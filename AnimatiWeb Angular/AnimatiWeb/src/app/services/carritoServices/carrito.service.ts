...
export class CarritoService {
  ...
  private getCarrito(): Observable<number | null> {
    ...
    return this.http
      .get<Carrito>(
        environment.API_END_POINT + environment.METHODS.GET_CART_USER,
        { headers }
      )
      .pipe(
  ...
  agregarProducto(producto: Producto): Observable<any> {
    if (this.carritoActivoId) {
      return this.createProductoCarrito(
        producto.Codigo_Producto,
        producto.Cantidad
      ).pipe(tap(() => this.actualizarProductosCarrito()));
    } else {
      return this.createCarrito().pipe(
        tap((carrito) => (this.carritoActivoId = carrito.id)),
        switchMap(() =>
          this.createProductoCarrito(
            producto.Codigo_Producto,
            producto.Cantidad
          )
        ),
  ...
  createCarrito() {
    ...
    return this.http.post<Carrito>(
      environment.API_END_POINT + environment.METHODS.CREATE_CART,
      {},
      { headers }
    );
  }
  ...
  createProductoCarrito(codigo: number, cantidad: number) {
    ...
    return this.http.post(
      environment.API_END_POINT + environment.METHODS.CREATE_PRODUCT_CART,
      body,
      { headers }
    );
  }
  ...
  getProductosCarrito() {
    ...
    return this.http.get<ProductoCarrito[]>(
      environment.API_END_POINT +
        environment.METHODS.GET_PRODUCT_CART_BY_ID +
        (this.carritoActivoId || 0),
      { headers }
    );
  }
  ...
  private actualizarCantidadLocal(
  ...
  deleteProducto(producto: ProductoCarrito) {
    ...
    return this.http
      .delete(
        environment.API_END_POINT +
          environment.METHODS.DELETE_PRODUCT_CART_BY_ID +
          producto.id,
        { headers }
      )
      .pipe(
  ...
  deleteUnProducto(producto: ProductoCarrito): Observable<any> {
    ...
    return this.http
      .patch(
        environment.API_END_POINT +
          environment.METHODS.DELETE_ONE_PRODUCT_CART_BY_ID +
          producto.id,
        {},
        { headers }
      )
  ...
  createPreference() {
    ...
    return this.http.post<Preferencia>(
      environment.API_END_POINT + environment.METHODS.CREATE_PREFERENCE,
      {},
      { headers }
    );
  }
